...
names: [Eleanor Pak Wai Lam, Jiya Shah, Isabella Monaghan Chow, Khubaib Choudry]
...
The BrainAGE Calculator project utilizes a range of tools and platforms to support development, reproducibility, and open dissemination:
...
### Data
This project uses normative neuroimaging data from the Reproducible Brain Charts initiative, specifically the Healthy Brain Network (HBN) subset. The HBN provides openly available structural MRI features from a large developmental cohort (ages 5–21).
...
* Git & GitHub: Collaborating on RMarkdown scripts and resolving issues highlighted the critical role of Git. I gained practical experience using Git for tracking changes and GitHub for team collaboration, particularly in troubleshooting and merging.
* Bash/Terminal: I developed proficiency in fundamental command-line operations (e.g., cd, ls, mkdir) for navigating directories, managing files, and organizing project outputs, which was essential for handling data and script execution.
* R Programming: This project provided my first opportunity to use R extensively, allowing me to develop in-depth skills in data wrangling, scripting, and leveraging its capabilities for complex data manipulation and analysis within the project's scope.

### Results

#### Deliverable: A Github repository

We developed a suite of RMarkdown documents to standardize and simplify data preparation for open-access BrainAGE calculators. These tools are publicly hosted in a centralized GitHub repository, providing researchers with easy access to reproducible workflows and documentation.

Key contributions include:

- HBN RBC Data Preparation Guide: An RMarkdown document (HBN_data.Rmd) offering a reproducible pipeline to download structural MRI data from the Healthy Brain Network Reproducible Brain Charts (HBN RBC) dataset, aggregate files into CSVs, and reformat regional surface statistics for compatibility with multiple brain atlases and BrainAGE models.

- Calculator-Specific Data Wrangling Scripts: Seven R scripts tailored to transform demographic and MRI data into the specific input formats required by individual BrainAGE calculators. These scripts automate column mapping, renaming, type conversions, and output generation to facilitate seamless integration.
...
## Acknowledgement
I would like to thank BrainHack School 2025 for organizing this course and providing a supportive learning environment. As someone new to coding, I faced many challenges, and I am especially grateful to my group members, course instructor, and TA for their patience, guidance, and encouragement throughout the project.
